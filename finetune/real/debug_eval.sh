...
# Number of episodes to sample **per task**.  Override with the env-var or
# pass --num_episodes directly on the command line.
NUM_EPISODES="${NUM_EPISODES:-3}"
NUM_EP_ARGS=()
if [[ " $* " != *" --num_episodes "* ]]; then
    NUM_EP_ARGS+=(--num_episodes "${NUM_EPISODES}")
fi
